...
(function ($) {
  ...
  BootstrapTable.prototype.initData = function () {
    ...
    //初始化数据，添加leavel,isLeaf 属性
    if (that.options.treeView && that.options.searchText == "" && this.data.length > 0) {
      var rows = [];
      var leavelStep=def_leavel;

      // this.data.forEach(function (val) {
      //   if(leavelStep>val.belong){
      //     leavelStep = val.belong
      //   }
      // })


      var roots = $.grep(this.data, function (row, index) {
        return row.belong == null || row.belong == leavelStep;
      });
      var g = function (child) {
        // var childLeavel=that.options.treeRootLeavel+leavelStep;
        $.each(child, function (i, n) {
          // n.leavel=childLeavel;
      ...
      $.each(roots, function (index, item) {
        item.leavel = that.options.treeRootLeavel;
    ...
    $.each(this.header.fields, function (j, field) {
      ...
      if (column.checkbox || column.radio) {
        ...
        if (that.options.treeView && column.field == that.options.treeField) {
          if(that.options.searchText == ""){
            indent = item.leavel == that.options.treeRootLeavel ? '' : sprintf('<span style="margin-left: %spx;"></span>', (item.leavel - that.options.treeRootLeavel) * 15);
            var child = $.grep(data, function (d, i) {
              return d.belong == item[that.options.treeId] && !d.hidden;
            });
  ...
  BootstrapTable.prototype.initBody = function (fixedScroll) {
    ...
    this.$body.find('> tr[data-index] > td > .tree-icon').off('click').on('click', function (e) {
      ...
      if (icon.hasClass(that.options.expandIcon)) {
        ...
        child = getAllChild(data[index], data, that.options.treeId);
        ...
        child = getChild(data[index], data, that.options.treeId);
        $.each(child, function (i, c) {
          $.each(that.data, function (index, item) {
            if (item[that.options.treeId] == c[that.options.treeId]) {
              item.hidden = false;
  ...
  BootstrapTable.prototype.expandAllTree = function () {
    var that = this;
    var roots = $.grep(this.data, function (row, index) {
      return row.belong == null || row.belong == -1;
    });
    $.each(roots, function (index, item) {
  ...
  $.extend($.fn.bootstrapTable.defaults, {
    ...
    treeId: "id",
    treeRootLeavel: 0, //根节点序号
    treeCollapseAll: true, //是否全部展开，默认不展开
    collapseIcon: "fa fa-caret-right fa-fw", //折叠样式
    expandIcon: "fa fa-caret-down fa-fw", //展开样式
    leafIcon: "fa fa-stop-circle-o fa-fw" //叶子节点样式
  });
