...
<script>
  ...
  import { productPaginatorDetail, storeProduct, brandList } from '../../api/product'
  export default {
    name: 'productEdit',
    data() {
    ...
    methods: {
      ...
      getBrand(isLast = true) {
        brandList(this.brandMore).then(response => {
          ...
          if (isLast) {
            ...
            this.brandStatus = brandList.status
          }
      ...
      changeBrand(val) {
        if (val === '正常' && this.brandStatus === '停用') {
          messageBox.selfConfirm('商品所属品牌已停用，请启用品牌后再修改商品状态.')
          this.productForm.status = '停用'
        }
      },
      // 获取商品详情
      getDetail(type) {
        productPaginatorDetail(this.listQuery, this.more).then(response => {
          this.productForm = response.data.productPaginator.items[0]
          if (this.productForm.quality_period) {
            this.$set(this.productForm, 'quality_period_num', parseFloat(this.productForm.quality_period))
            this.$set(this.productForm, 'quality_period_unit', this.productForm.quality_period.slice(-1))
          }
          this.oldProductForm = _.cloneDeep(this.productForm)
          this.productForm.brand_id ? this.brandMore.id = this.productForm.brand_id : ''
          this.getBrand()
          if (this.productForm.pictures) {
            this.productForm.pictures.forEach((val) => {
              var obj = {}
              obj.url = val
              this.picturesImage.push(obj)
            })
          }
          if (this.productForm.check_pictures) {
            this.productForm.check_pictures.forEach((val) => {
              var obj = {}
              obj.url = val
              this.checkPictures.push(obj)
            })
          }
          // 页面渲染之后监听单品数量
          this.$watch('productForm.single_num', function(newVal, oldVal) {
            // 编辑时修改单品数量的价格联动
            if (newVal && this.oldProductForm.single_num && this.isEdit) {
              const val = newVal / this.oldProductForm.single_num
              this.productForm.one_price = this.fomatFloat(val * this.oldProductForm.one_price)
              this.productForm.two_price = this.fomatFloat(val * this.oldProductForm.two_price) || ''
              this.productForm.three_price = this.fomatFloat(val * this.oldProductForm.three_price) || ''
              this.productForm.four_price = this.fomatFloat(val * this.oldProductForm.four_price) || ''
              this.productForm.five_price = this.fomatFloat(val * this.oldProductForm.five_price) || ''
            }
          }, { deep: true })
        }).finally(() => {
        })
