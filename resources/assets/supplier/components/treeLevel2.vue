<template>
  <div class="tree-level">
    ...
    <div class="tree-item"
         :class="{'item-one': item._level === 1, 'item-two': item._level === 2, 'last-level': item._level === 3}"
         v-for="(item, index) in formatData2()"
         :style="showRow(item)"
         :key="item.id"
      ...
      <span class="item-rt" v-if="(item._level<3 && brandType!=='部门')||(item._level<3 && brandType==='部门' && !item.is_tip)" @click.stop="addLevel(item, numArray[item._level] + '级')">+添加{{ numArray[item._level] }}级{{ brandType }}</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    formatData2() {
      console.log(22222, this.formatData)
      let tmp
  ...
  watch: {
    treeData: {
      handler: function(newVal, oldVal) {
        ...
        this.formatData = this._.cloneDeep(newVal)
        this.formatData2()
        // this.$forceUpdate()
      },
      deep: true
